...
- **Rule Triggered On Create** — when a new record is created matching the target model; trigger type is on_create or on_create_or_write; the new record matches the filter condition (if set), then Actions executed automatically on the new record.
...
- **Rule Triggered On Message** — when a message is received on (or sent from) a record; the record type supports messaging; trigger type is on_message_received or on_message_sent, then Incoming/outgoing message triggers automated response.
