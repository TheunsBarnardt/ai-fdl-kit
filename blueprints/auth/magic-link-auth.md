...
- **Send Magic Link** — when Email is valid format; User with this email exists; Account is not disabled, then show "If an account exists with this email, we sent a sign-in link." (same message always). *(error: `MAGIC_LINK_VALIDATION_ERROR`)*
- **Send Magic Link No Account** — when Email is valid format; No user with this email exists, then show "If an account exists with this email, we sent a sign-in link." (SAME message — enumeration prevention). *(error: `MAGIC_LINK_ACCOUNT_DISABLED`)*
