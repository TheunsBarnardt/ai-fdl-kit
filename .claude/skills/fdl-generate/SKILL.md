...
### Step 0a.5: Project DESIGN.md overlay (always checked)

Independently of any prompt keyword, check the project root (cwd) and — if the user passed `--path` or you're inside a target generated by `/fdl-build` — the target directory for a `DESIGN.md` file. This file is produced by `/fdl-extract-design` and acts as the brand/design-system source of truth.

If `DESIGN.md` exists:
1. Read it. Parse the nine Stitch-format sections (Visual Theme, Color Palette, Typography, Components, Layout, Depth, Do/Don't, Responsive, Agent Prompt Guide).
2. Populate `extracted.design_tokens` from §2 colors, §3 typography, §5 spacing, §6 shadows, and the radius/component notes in §4 — **only if the slot is not already filled** by a Figma / image / code extraction above. Figma data wins when both are present.
3. Carry §7 "Do's and Don'ts" verbatim into the final generation prompt as hard constraints on the UI code you emit. Treat them at the same level as blueprint `rules`.
4. Use §9 "Agent Prompt Guide" as the literal prelude when you generate UI files, so hand-off to downstream agents (Cursor, Copilot) stays consistent.
5. Set `extracted.source = "design-md"` (or append to a list if another source already set it) and record the file path in `extracted.source_url_or_path` for the audit comment.

If no `DESIGN.md` exists, proceed with framework defaults — do not prompt the user to create one here. `/fdl-build` handles that suggestion, not this skill.
