...
- **Mobile Order Placed** — when customer scans table QR code containing table identifier and access token; an active POS session exists; customer selects products from the available menu, then Order appears on POS terminal and is sent to kitchen.
