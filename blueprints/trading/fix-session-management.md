...
- **sender_comp_id** *(text, required)*
- **target_comp_id** *(text, required)*
- **begin_string** *(select, required)*
- **session_qualifier** *(text, optional)*
- **heartbeat_interval** *(number, required)*
- **logon_timeout** *(number, optional)*
- **logout_timeout** *(number, optional)*
- **next_sender_seq_num** *(number, optional)*
- **next_target_seq_num** *(number, optional)*
- **reset_seq_num_flag** *(boolean, optional)*
- **default_appl_ver_id** *(text, optional)*
...
- **Sequence Gap Detected** — when message received with sequence number higher than expected, then ResendRequest sent for the missing range; message queued until gap filled.
