...
- **salt** *(hidden, optional)*
- **hash** *(hidden, optional)*
- **reset_password_token** *(token, optional)*
- **reset_password_expiration** *(datetime, optional)*
- **login_attempts** *(number, optional)*
- **lock_until** *(datetime, optional)*
- **verification_token** *(token, optional)*
- **verified** *(boolean, optional)*
- **sessions** *(json, optional)*
- **enable_api_key** *(boolean, optional)*
- **api_key** *(token, optional)*
- **api_key_index** *(hidden, optional)*
...
- **Api Key Auth** — when Authorization header contains '{collectionSlug} API-Key {key}'; HMAC-SHA256 of key matches stored apiKeyIndex; email is verified (if verification enabled), then User authenticated with _strategy set to 'api-key'.
