...
_Last run: 2026-04-11 — 23 blueprints below the fitness threshold. Pick a row, paste the command into Claude Code, and `/fdl-auto-evolve` will re-score after extraction._

### 🔴 integration/identity-brokering-social-login — 59/100
**Weakest dimension:** `relationships` (0/10) — no relationships

- **nextauthjs/next-auth** — Social login providers (Google, GitHub, Facebook, ...) are declarative
  ```
  /fdl-extract-code https://github.com/nextauthjs/next-auth identity-brokering-social-login integration
  ```
- **keycloak/keycloak** — Identity brokering to external IdPs is first-class
  ```
  /fdl-extract-code https://github.com/keycloak/keycloak identity-brokering-social-login integration
  ```
- **ory/hydra** — Canonical OAuth2/OIDC reference
  ```
  /fdl-extract-code https://github.com/ory/hydra identity-brokering-social-login integration
  ```
