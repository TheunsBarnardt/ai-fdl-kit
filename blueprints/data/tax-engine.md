...
- **Cash Basis Deferred** — when tax_exigibility eq "on_payment"; invoice is posted but payment not yet received, then Tax liability deferred until payment. On payment reconciliation, amount moves from transition account to final tax account. *(error: `TAX_CASH_BASIS_NO_ACCOUNT`)*
