...
- **destination_key** *(text, optional)*

## What must be true

- **0:** Set elements are unique; adding duplicate element replaces (no effect on cardinality)
- **1:** Set membership is unordered (no indices or ranges)
- **2:** Set algebra operations (intersection, union, difference) can accept multiple source sets
- **3:** Intersection of multiple sets returns elements present in ALL sets
- **4:** Union of multiple sets returns elements present in ANY set
- **5:** Difference of first set minus others returns elements in first but not in any other
- **6:** Store variants (*STORE) create destination set atomically; overwrite if exists
