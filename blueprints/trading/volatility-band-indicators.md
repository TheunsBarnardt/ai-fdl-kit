...
- **Compute Success** — when input_length > lookback_period for selected indicator; required price arrays present; parameters within valid ranges, then Volatility series aligned to input via outBegIdx.
- **Bbands Computed** — when indicator_type == BBANDS; input_length > (timePeriod - 1), then Three overlaid bands; bandwidth contraction (squeeze) precedes breakout.
- **Volatility Expansion** — when indicator_type in [ATR, NATR]; out_values[latest] > 1.5 * mean(out_values[-20:]), then Volatility expansion signal — widen stops or reduce position size.

**❌ Failure paths**

- **Insufficient Data** — when input_length <= lookback_period, then No valid output; buffer more bars. *(error: `INSUFFICIENT_DATA`)*
- **Invalid Parameters** — when time_period < 1 and indicator_type in [ATR, NATR] OR time_period < 2 and indicator_type in [BBANDS, STDDEV, VAR] OR nb_dev_up == 0 and nb_dev_dn == 0 and indicator_type == BBANDS, then Function returns error code; validate parameters before calling. *(error: `INVALID_PARAMETER`)*
- **Natr Zero Close** — when indicator_type == NATR; any close_prices value == 0, then NATR output undefined at that bar; caller must filter zero-price bars. *(error: `INVALID_INPUT`)*
