...
- **lead_source** *(select, optional)*
- **lead_notes** *(rich_text, optional)*
- **lead_status** *(select, optional)*
...
- **Send Message** — when client_id exists; message_body exists; client_id in "assigned_clients"; user.role eq "IFA", then Message sent to client with notification.
...
- **Update Lead Status** — when lead_id exists; lead_status exists; user.role eq "IFA", then Lead status updated.
