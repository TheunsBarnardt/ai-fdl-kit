...
**`deposit.success`** — Deposit confirmed on the blockchain
  Payload: `wallet_id`, `address_id`, `amount`, `asset_id`, `transaction_hash`, `metadata`

**`deposit.processing`** — Deposit detected, awaiting confirmation
  Payload: `wallet_id`, `address_id`, `amount`, `asset_id`

**`deposit.failed`** — Deposit failed to be processed
  Payload: `wallet_id`, `address_id`, `error_code`

**`deposit.swept.success`** — Funds auto-swept from child address to master wallet
  Payload: `wallet_id`, `address_id`, `amount`, `transaction_hash`

**`deposit.swept.failed`** — Auto-sweep failed (likely insufficient gas)
  Payload: `wallet_id`, `address_id`, `error_code`

**`withdraw.success`** — Withdrawal confirmed on blockchain
  Payload: `wallet_id`, `amount`, `asset_id`, `destination_address`, `transaction_hash`

**`withdraw.failed`** — Withdrawal failed
  Payload: `wallet_id`, `error_code`

**`offramp.processing`** — Fiat withdrawal being processed
  Payload: `wallet_id`, `amount`, `currency`

**`offramp.success`** — Fiat withdrawal completed
  Payload: `wallet_id`, `amount`, `currency`, `institution_identifier`

**`offramp.failed`** — Fiat withdrawal failed
  Payload: `wallet_id`, `error_code`

**`gateway.deposit.success`** — USDC deposited to Gateway unified balance
  Payload: `wallet_id`, `amount`, `source_chain`

**`gateway.withdraw.success`** — USDC withdrawn from Gateway to destination chain
  Payload: `wallet_id`, `amount`, `destination_chain`

**`swap.success`** — Asset swap executed successfully
  Payload: `wallet_id`, `from_asset`, `to_asset`, `amount`

**`swap.failed`** — Swap transaction failed
  Payload: `wallet_id`, `error_code`

**`signed.success`** — Transaction signed and broadcast successfully
  Payload: `wallet_id`, `transaction_hash`

**`signed.failed`** — Transaction broadcast failed after all retries
  Payload: `wallet_id`, `error_code`

**`salvage.success`** — Asset recovery completed
  Payload: `wallet_id`, `amount`, `blockchain`

**`salvage.failed`** — Asset recovery failed
  Payload: `wallet_id`, `error_code`
