...
| [Fleet Device Sharing]({{ site.baseurl }}/blueprints/access/fleet-device-sharing/) | Control which users can see and operate which GPS devices through an ACL permission model, with hierarchical device groups that inherit configuration and enable bulk sharing, user restrictions to limit dangerous operations, and manager-level user provisioning. | 1.0.0 |
