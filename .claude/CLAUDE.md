...
```bash
# Regenerate slim retrieval index for skills (fast, ~5s)
...
- **Co-located readmes:** `blueprints/{category}/{feature}.md` — a concise, human-friendly summary auto-generated next to each blueprint by `scripts/generate-readmes.js`. YAML stays the source of truth; these `.md` files are **for humans skimming the repo only — skills must never load them** (they duplicate content already in the YAML and inflate context). Overwritten on every `npm run generate`. Never hand-edit them.
- **Blueprint headers index:** `blueprints/blueprint-headers.json` — compact retrieval index (262 KB) for skills. Keys per entry: `id` (feature), `c` (category), `d` (description oneline), `a` (aliases), `t` (tags), `r` (related IDs). **Skills load this instead of INDEX.md for match/filter phases.** Full YAML is only read for confirmed candidates. Regenerated by `npm run generate` or `npm run generate:headers`.
- **Blueprint index:** `blueprints/INDEX.md` — human-readable lookup file listing every blueprint grouped by category. For developers browsing the repo. Skills should prefer `blueprint-headers.json` (structured JSON, programmatically filterable). Regenerated by `scripts/generate-readmes.js`.
