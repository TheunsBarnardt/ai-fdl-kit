...
| [Fleet Scheduled Reports]({{ site.baseurl }}/blueprints/workflow/fleet-scheduled-reports/) | Generate, schedule, and distribute fleet tracking reports covering trips, stops, route history, events, geofence activity, device summaries, and fuel consumption, with on-demand and automated periodic delivery. | 1.0.0 |
...
| [Maintenance Reminders]({{ site.baseurl }}/blueprints/workflow/maintenance-reminders/) | Define maintenance tasks that trigger notifications when a tracked vehicle crosses a configured odometer, engine hours, or time threshold, with automatic repeat reminders at regular intervals for ongoing maintenance schedules. | 1.0.0 |
...
| [Trip Detection]({{ site.baseurl }}/blueprints/workflow/trip-detection/) | Automatically detect the start and end of vehicle trips by monitoring movement patterns across consecutive position records, applying configurable distance and duration thresholds to filter noise, and producing structured trip records for reporting. | 1.0.0 |
