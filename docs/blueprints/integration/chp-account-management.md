---
title: "Chp Account Management Blueprint"
layout: default
parent: "Integration"
grand_parent: Blueprint Catalog
description: "Account management services for CHP — account mirroring, PayShap proxy management, real-time account verification (AVS-R), and check digit verification (CDV). 1"
---

# Chp Account Management Blueprint

> Account management services for CHP — account mirroring, PayShap proxy management, real-time account verification (AVS-R), and check digit verification (CDV)
...
| **Version** | 1.0.0 |
| **Tags** | chp, electrum, bankservafrica, payshap, proxy, avsr, cdv, account-mirror, payments, south-africa |
...
| **JSON API** | [chp-account-management.json]({{ site.baseurl }}/api/blueprints/integration/chp-account-management.json) |

## Actors

| ID | Name | Type | Description |
|----|------|------|-------------|
| `partner_system` | Partner System | system | Financial institution integrating with Electrum for CHP services |
| `electrum` | Electrum Platform | external | Electrum payment orchestration platform — manages CHP integration layer |
| `bankservafrica` | BankservAfrica | external | South African automated clearing house — operates PayShap, RTC, and EFT schemes |
...
- **account_mirror:**
  - **use_case_a:** Electrum as Secondary Store — partner manages proxies, Electrum mirrors for faster routing
  - **use_case_b:** Electrum as Primary Store — Electrum manages proxies, partner mirrors for local resolution
  - **use_case_c:** No Electrum Mirror — partner handles all proxy and account resolution directly
  - **use_case_d:** Electrum as RTC Account Mirror — for RTC-only institutions
...
- **payshap_proxy:**
  ...
  - **resolution_mandatory:** Proxy resolution is mandatory for PayShap inbound payments
...
- **avsr:**
  ...
  - **outbound:** Partner sends verification request to Electrum
  - **inbound:** Electrum sends verification request to partner
  - **time_limit:** AVS-R responses must be returned within scheme time limits
- **cdv:**
  - **description:** BankservAfrica Check Digit Verification for SA bank accounts
  - **validation_required:** Account numbers must pass CDV validation before payment initiation
- **account_types:**
  - **standard:** Account types follow BankservAfrica standards: OTHER, CURRENT, SAVINGS, TRANSMISSION, BOND, SUBSCRIPTION_SHARE
- **identification:**
...
**Then:**
- **call_service** target: `electrum_api.resolve_proxy` — Resolve proxy to its corresponding bank account via Electrum
...
**Then:**
- **call_service** target: `electrum_api.update_account_mirror` — Synchronize account data to Electrum mirror via AccountUpdateRequest
- **emit_event** event: `account.mirror.updated`

**Result:** Account data synchronized to Electrum mirror — routing information up to date
...
**Then:**
- **call_service** target: `electrum_api.delete_account_mirror` — Remove account from Electrum mirror
- **transition_state** field: `account_status` from: `enabled` to: `deleted`
- **emit_event** event: `account.mirror.deleted`

**Result:** Account removed from Electrum mirror — no longer available for routing
...
**Then:**
- **call_service** target: `electrum_api.verify_account_avsr` — Submit AVS-R verification request to BankservAfrica via Electrum
...
**Then:**
- **call_service** target: `electrum_api.validate_cdv` — Submit account number for BankservAfrica check digit verification
...
|-------|-------------|----------|
| `account.mirror.updated` | Account data synchronized to Electrum mirror | `account_number`, `account_status`, `record_identifier` |
| `account.mirror.deleted` | Account removed from Electrum mirror | `account_number`, `record_identifier` |
...
| chp-request-to-pay | recommended | Proxy resolution for PayShap RTP addressing |
...
<script type="application/ld+json">
{
  ...
  "name": "Chp Account Management Blueprint",
  "description": "Account management services for CHP — account mirroring, PayShap proxy management, real-time account verification (AVS-R), and check digit verification (CDV). 1",
  "programmingLanguage": "YAML",
  "codeRepository": "https://github.com/TheunsBarnardt/claude-fdl",
  "license": "https://opensource.org/licenses/MIT",
  "keywords": "chp, electrum, bankservafrica, payshap, proxy, avsr, cdv, account-mirror, payments, south-africa"
}
</script>
