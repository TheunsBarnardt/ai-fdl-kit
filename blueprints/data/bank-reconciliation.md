...
- **Line Auto Reconciled** — when a reconciliation model with auto trigger exists; statement line matches all model conditions (amount, label, partner), then Statement line automatically matched and reconciled. *(error: `BANK_LINE_ALREADY_RECONCILED`)*
