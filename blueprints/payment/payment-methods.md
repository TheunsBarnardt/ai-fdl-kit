...
- **Payment Method Added** — when customer submits card details via secure form; card passes Luhn validation; payment provider returns a valid token; card fingerprint is not a duplicate, then Payment method tokenized and saved securely. *(error: `PAYMENT_METHOD_INVALID_CARD`)*
...
- **Payment Method Removed** — when customer requests removal of a payment method; payment method is not the sole method on an active subscription, then Payment method removed and token revoked. *(error: `PAYMENT_METHOD_EXPIRED`)*
