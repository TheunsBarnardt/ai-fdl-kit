...
_Last run: 2026-04-11 — 39 blueprints below the fitness threshold. Pick a row, paste the command into Claude Code, and `/fdl-auto-evolve` will re-score after extraction._

### 🔴 notification/device-alarm-notifications — 48/100
**Weakest dimension:** `rules` (0/10) — rules missing or flat

_No candidates mapped for `notification` + `rules`. Run `/fdl-recommend-discover device-alarm-notifications` to find some._

### 🔴 data/fuel-level-reporting — 49/100
**Weakest dimension:** `rules` (0/10) — rules missing or flat
...
### 🔴 workflow/maintenance-reminders — 49/100
**Weakest dimension:** `rules` (0/10) — rules missing or flat

_No candidates mapped for `workflow` + `rules`. Run `/fdl-recommend-discover maintenance-reminders` to find some._

### 🔴 data/driver-identification — 50/100
**Weakest dimension:** `rules` (0/10) — rules missing or flat
...
### 🔴 notification/device-power-alerts — 50/100
**Weakest dimension:** `rules` (0/10) — rules missing or flat

_No candidates mapped for `notification` + `rules`. Run `/fdl-recommend-discover device-power-alerts` to find some._

### 🔴 workflow/stop-detection — 50/100
**Weakest dimension:** `rules` (0/10) — rules missing or flat

_No candidates mapped for `workflow` + `rules`. Run `/fdl-recommend-discover stop-detection` to find some._

### 🔴 data/engine-hours-tracking — 52/100
**Weakest dimension:** `rules` (0/10) — rules missing or flat
...
### 🔴 workflow/trip-detection — 54/100
**Weakest dimension:** `rules` (0/10) — rules missing or flat

_No candidates mapped for `workflow` + `rules`. Run `/fdl-recommend-discover trip-detection` to find some._

### 🔴 access/fleet-device-sharing — 57/100
**Weakest dimension:** `rules` (0/10) — rules missing or flat

- **openfga/openfga** — Fine-grained authorization — rules encoded as relationship tuples, Google Zanzibar-inspired
  ```
  /fdl-extract-code https://github.com/openfga/openfga fleet-device-sharing access
  ```
- **cerbos/cerbos** — Policy-as-code authorization engine — richly structured, typed rules in YAML
  ```
  /fdl-extract-code https://github.com/cerbos/cerbos fleet-device-sharing access
  ```
- **casbin/casbin** — PERM metamodel access control — policy definitions are declarative
  ```
  /fdl-extract-code https://github.com/casbin/casbin fleet-device-sharing access
  ```

### 🔴 notification/geofence-alerts — 58/100
...
### 🔴 data/device-status-tracking — 60/100
**Weakest dimension:** `agi` (0/5)
...
### 🔴 notification/overspeed-alerts — 60/100
**Weakest dimension:** `agi` (0/5)
...
### 🔴 data/ignition-detection — 61/100
**Weakest dimension:** `agi` (0/5)
...
### 🔴 integration/remote-device-commands — 64/100
**Weakest dimension:** `agi` (0/5)
...
### 🔴 workflow/fleet-scheduled-reports — 64/100
**Weakest dimension:** `rules` (0/10) — rules missing or flat

_No candidates mapped for `workflow` + `rules`. Run `/fdl-recommend-discover fleet-scheduled-reports` to find some._
...
### 🔴 data/geofence-management — 68/100
**Weakest dimension:** `agi` (0/5)

- **payloadcms/payload** — Node headless CMS — rich data modeling, access control, hooks
  ```
  /fdl-extract-code https://github.com/payloadcms/payload geofence-management data
  ```
- **directus/directus** — Database-first data platform — collections, fields, relations, flows
  ```
  /fdl-extract-code https://github.com/directus/directus geofence-management data
  ```
...
### 🔴 data/gps-device-registration — 69/100
**Weakest dimension:** `agi` (0/5)

- **payloadcms/payload** — Node headless CMS — rich data modeling, access control, hooks
  ```
  /fdl-extract-code https://github.com/payloadcms/payload gps-device-registration data
  ```
- **directus/directus** — Database-first data platform — collections, fields, relations, flows
  ```
  /fdl-extract-code https://github.com/directus/directus gps-device-registration data
  ```
