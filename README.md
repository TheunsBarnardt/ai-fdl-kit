...
_Last run: 2026-04-12 — 56 blueprints below the fitness threshold. Pick a row, paste the command into Claude Code, and `/fdl-auto-evolve` will re-score after extraction._
...
### 🔴 workflow/driver-shift-management — 62/100
**Weakest dimension:** `error_binding` (2/10) — 2 of 2 codes unbound

_No candidates mapped for `workflow` + `error_binding`. Run `/fdl-recommend-discover driver-shift-management` to find some._
...
### 🔴 integration/webhook-trip-lifecycle — 67/100
**Weakest dimension:** `error_binding` (2/10) — 2 of 2 codes unbound

- **ory/hydra** — OAuth2 error codes (invalid_request, invalid_grant, ...) bind 1:1 to handler branches
  ```
  /fdl-extract-code https://github.com/ory/hydra webhook-trip-lifecycle integration
  ```
- **panva/node-oidc-provider** — OIDC spec error codes are typed and returned from specific flow steps
  ```
  /fdl-extract-code https://github.com/panva/node-oidc-provider webhook-trip-lifecycle integration
  ```
- **keycloak/keycloak** — Canonical identity brokering reference
  ```
  /fdl-extract-code https://github.com/keycloak/keycloak webhook-trip-lifecycle integration
  ```
...
### 🔴 data/eta-calculation — 68/100
**Weakest dimension:** `events` (2/5)

- **payloadcms/payload** — Node headless CMS — rich data modeling, access control, hooks
  ```
  /fdl-extract-code https://github.com/payloadcms/payload eta-calculation data
  ```
- **directus/directus** — Database-first data platform — collections, fields, relations, flows
  ```
  /fdl-extract-code https://github.com/directus/directus eta-calculation data
  ```
