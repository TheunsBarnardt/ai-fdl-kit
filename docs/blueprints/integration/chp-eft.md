---
title: "Chp Eft Blueprint"
layout: default
parent: "Integration"
grand_parent: Blueprint Catalog
description: "Electronic Funds Transfer operations via Electrum's Clearing House Payments platform — inbound/outbound credits, debits, returns, on-us debits, payment cancella"
---

# Chp Eft Blueprint

> Electronic Funds Transfer operations via Electrum's Clearing House Payments platform — inbound/outbound credits, debits, returns, on-us debits, payment cancellation, and system error correction (SEC)
...
| **Version** | 1.0.0 |
| **Tags** | eft, payments, clearing-house, bankservafrica, south-africa, credits, debits, returns, sec, electrum |
...
| **JSON API** | [chp-eft.json]({{ site.baseurl }}/api/blueprints/integration/chp-eft.json) |

## Actors

| ID | Name | Type | Description |
|----|------|------|-------------|
| `partner_system` | Partner System | system | Financial institution integrating with Electrum for EFT processing |
| `electrum` | Electrum Platform | external | Electrum payment orchestration platform that mediates between partner and BankservAfrica |
| `bankservafrica` | BankservAfrica | external | South African automated clearing house operator for EFT batch settlement |
...
- **settlement:**
  - **batch_processing:** EFT transactions are batch-processed through BankservAfrica — not real-time
...
- **cancellation:**
  ...
  - **resolution:** Electrum delivers cancellation resolution via callback
- **sec:**
...
**Given:**
- Electrum sends inbound credit notification
- `creditor_account_number` (input) exists
- `amount_value` (input) gt `0`
...
**Given:**
- Electrum sends inbound debit notification
- `debtor_account_number` (input) exists
- `amount_value` (input) gt `0`
...
**Given:**
- Electrum sends inbound return notification
- `uetr` (input) exists
- `return_reason` (input) exists
...
**Given:**
- Electrum delivers cancellation resolution via callback
- `uetr` (input) exists
...
esm:
  description: EFT Settlement Module — BankservAfrica's batch settlement system
  record_types:
    - PAYMENT
    - UNPAID_INPUT
    - UNPAID_OUTPUT
...
<script type="application/ld+json">
{
  ...
  "name": "Chp Eft Blueprint",
  "description": "Electronic Funds Transfer operations via Electrum's Clearing House Payments platform — inbound/outbound credits, debits, returns, on-us debits, payment cancella",
  "programmingLanguage": "YAML",
  "codeRepository": "https://github.com/TheunsBarnardt/claude-fdl",
  "license": "https://opensource.org/licenses/MIT",
  "keywords": "eft, payments, clearing-house, bankservafrica, south-africa, credits, debits, returns, sec, electrum"
}
</script>
